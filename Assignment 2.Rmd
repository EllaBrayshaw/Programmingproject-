...
Looking at the plots for the subsections of the original grid; when $\lambda = 0.94$ I can there are 3 values of $\gamma$ that result in a type 1 error rate of less the 0.05 as before diregarding the point at zero as $\gamma >0.$ I can also rule out the option of $\lambda = 0.92$ as the only value of gamma in combination with this that leads to a low enough type 1 error is $\gamma = 0$. 
...
To decide a value of $\gamma$ to use in combination with $\lambda = 0.94$, I will consider the plot of the type II error rates against $\gamma$. The plot below shows that the type II error rate falls below the maximum value of 0.2 for all combinations, however you can see that ideally a larger choice of $\gamma$ will be preferred as type II error rate decreases as $\gamma$ increases. Hence to decide the value of $\gamma$ I shall look at the summary of the table earlier. 

```{r, echo = FALSE}
sub15 <- seq_T2s_60_bind[seq_T2s_60_bind$lambda == 0.94, ]
plot(sub15$gamma, sub15$T2s_60)
abline(h = 0.2, col = "red")
```
...
From this summary I see that for the desired maximum type I error to be met we require $\gamma \in (0,0.24]$, I will take $\gamma = 0.24$ as this results in bigger improvement to type II error without affecting type I error too much. This in combination with $\lambda = 0.94$ results in a type I error rat of 0.045 and a type II error rate of 0.011. These are both lower then the rates found for the smaller sample size, in particular the probability of type II error has decreased by quite a lot suggesting increasing sample size reduces the probability of stopping the trial early when results are actually significant.... 
